...
**ADDITIONAL: Git Credential Helper Not Configured (2026-05-16 discovery)**

If push works from one repo but fails from another with the same remote:
```
...
```bash
# Working repo
cd ~/command_center && git config --local credential.helper  # May show custom helper

# Failing repo  
cd ~/kiri && git config --local credential.helper  # Returns empty

# Fix
git config credential.helper gh  # Use gh CLI as credential helper
```

**Root cause:** The `gh` CLI is authenticated (`gh auth status` shows token) but git doesn't know to use it. Set `credential.helper gh` to bridge git operations through gh's auth.

**Also check for custom credential helpers:**
```bash
ls -la ~/.git-credential-*  # May exist but not be referenced in git config
```
---
